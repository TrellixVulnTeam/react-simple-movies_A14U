...
const MovieDetailPage = () => {
  ...
  return (
    <div className="py-10">
      ...
      <MovieMeta props="credits"></MovieMeta>
      <MovieMeta props="videos"></MovieMeta>
      <MovieMeta props="similar"></MovieMeta>
    </div>
  );
};
...
function MovieMeta({ props }) {
  const { movieId } = useParams();
  const { data } = useSWR(tmdbAPI.getMovieMeta(movieId, props), fetcher);
  if (!data) return null;
